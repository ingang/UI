...
# Windows 사용자를 위한 수정입니다. 이 수정 없이는 JavaScript 파일이 text/html 콘텐츠 유형으로 제공되어 브라우저에서 UI를 표시하지 않습니다.

mimetypes.init()
mimetypes.add_type('application/javascript', '.js')

# Gradio가 홈으로 전화하는 것을 고치고 IP 주소를 설정합니다.
if not cmd_opts.share and not cmd_opts.listen:
    # fix gradio phoning home
    gradio.utils.version_check = lambda: None
    gradio.utils.get_local_ip_address = lambda: '127.0.0.1'

# ngrok이 설정되어 있으면 ngrok을 사용하여 연결을 시도합니다.
...
#샘플이미지 존재하지않을경우 None으로 처리
sample_img2img = sample_img2img if os.path.exists(sample_img2img) else None

#진행 표시줄 및 관련 요소를 숨기는 CSS 스타일 정의
...
# 변형 선택기가 표시되지 않으므로 상수를 사용합니다.
# 툴팁이 작동하려면 script.js와 정확히 일치해야 합니다.
...
#plaintext_to_html: 텍스트를 HTML 형식으로 변환하는 함수입니다. 이 함수는 ui_common.plaintext_to_html 함수를 호출하여 작업을 수행합니다.

def plaintext_to_html(text):
    return ui_common.plaintext_to_html(text)

#send_gradio_gallery_to_image: Gradio 갤러리에서 선택한 이미지의 URL을 가져와 이미지로 변환하는 함수입니다. 이미지 URL이 없는 경우 None을 반환합니다
def send_gradio_gallery_to_image(x):
    if len(x) == 0:
        return None
    return image_from_url_text(x[0])

#visit: 재귀적으로 객체 트리를 탐색하여 주어진 함수를 적용하는 함수입니다. 객체가 'children' 속성을 가지고 있으면, 
#이 함수는 자식 객체를 반복하고 각각에 대해 'visit' 함수를 호출합니다. 그렇지 않으면 주어진 함수를 객체에 적용합니다.
...
#add_style: 스타일을 추가하는 함수입니다. 이름, 프롬프트, 부정적인 프롬프트를 인수로 사용합니다. 
#함수는 스타일을 생성하고 저장한 다음, 스타일 드롭다운의 선택 사항을 업데이트합니다.
...
def add_style(name: str, prompt: str, negative_prompt: str):
    ...
    # 로드된 모든 프롬프트 스타일 저장: 다음과 같은 이유로 향후 저장소 형식을 더 쉽게 업데이트할 수 있습니다.
    # 저장할 때마다 모든 스타일을 다시 직렬화합니다.
    shared.prompt_styles.save_styles(shared.styles_filename)

    return [gr.Dropdown.update(visible=True, choices=list(shared.prompt_styles.styles)) for _ in range(2)]

#calc_resolution_hires: 높은 해상도 옵션이 활성화되어 있는지 확인하고, 
#사용자가 지정한 높은 해상도 매개변수를 기반으로 해상도를 조정합니다.
...
#apply_styles: 주어진 스타일을 프롬프트와 부정적인 프롬프트에 적용하는 함수입니다. 
#이 함수는 각각의 프롬프트에 대해 적용된 스타일을 반환합니다.
...
#process_interrogate: 주어진 탐구 함수를 사용하여 이미지에 대한 정보를 추출하는 함수입니다. 
#이 함수는 다양한 모드를 지원하며, 이미지 입력 디렉토리와 출력 디렉토리에 따라 이미지를 처리합니다.
...
#interrogate: 입력 이미지를 변환하고 이미지에 대한 프롬프트를 생성합니다. 
# 프롬프트가 없으면 업데이트를 반환하고, 그렇지 않으면 프롬프트를 반환합니다.

def interrogate(image):
    prompt = shared.interrogator.interrogate(image.convert("RGB"))
    return gr.update() if prompt is None else prompt

#interrogate_deepbooru: 입력 이미지에 대한 태그를 생성합니다.
# 태그가 없으면 업데이트를 반환하고, 그렇지 않으면 태그를 반환합니다.

def interrogate_deepbooru(image):
    prompt = deepbooru.model.tag(image)
    return gr.update() if prompt is None else prompt

#create_seed_inputs: 사용자 인터페이스의 시드 및 변형 시드 구성 요소를 생성하고 반환하는 함수입니다.
...
#connect_clear_prompt: 주어진 'clear' 버튼에 대해 클릭 이벤트를 설정합니다.
#이 함수는 프롬프트와 토큰 카운터 객체를 지우는 기능을 처리합니다.
...
#connect_reuse_seed: 'reuse (sub)seed' 버튼의 클릭 이벤트를 연결하여 생성 정보에서 마지막으로 사용된 (sub)seed 값을 seed 필드에 복사합니다.
#만약 subseed를 복사하고 subseed 강도가 0이면, 즉 변형 시드가 사용되지 않은 경우 대신 일반 시드 값을 복사합니다.
#이 두 함수는 사용자 인터페이스에 작동하는 이벤트를 연결합니다. connect_clear_prompt 함수는 주어진 'clear' 버튼을 클릭하면 프롬프트를 지우도록 설정하고, 
#connect_reuse_seed 함수는 'reuse (sub)seed' 버튼을 클릭하면 이전에 사용된 시드 값을 복사하여 사용자 인터페이스에 입력될 시드 값으로 설정합니다.
...
#이 함수는 주어진 텍스트와 단계를 사용하여 토큰 카운터를 업데이트합니다. 
#이 함수의 목적은 프롬프트의 토큰 수를 계산하고 사용자 인터페이스에 표시하는 것입니다.
...
#create_toprow 함수는 사용자 인터페이스의 맨 위 행을 생성하며, 
#여기에는 img2img 또는 txt2img 모드에 대한 기본 입력 필드와 작업 버튼이 포함되어 있습니다.
...
def create_toprow(is_img2img):
    ...
    with gr.Row(elem_id=f"{id_part}_toprow", variant="compact"):
        ...
        with gr.Column(scale=1, elem_id=f"{id_part}_actions_column"):
            # 인터럽트, 건너뛰기, 생성 버튼 추가하기
            with gr.Row(elem_id=f"{id_part}_generate_box"):
                interrupt = gr.Button('Interrupt', elem_id=f"{id_part}_interrupt")
                skip = gr.Button('Skip', elem_id=f"{id_part}_skip")
                submit = gr.Button('Generate', elem_id=f"{id_part}_generate", variant='primary')
                
                # 건너뛰기 및 인터럽트 버튼을 해당 기능에 연결합니다.
                skip.click(
            ...
            # 붙여넣기, 프롬프트 지우기, 추가 네트워크, 프롬프트 스타일 적용 및 스타일 저장 버튼을 추가합니다.
...
# 진행률 표시 줄 설정 함수 (현재 아무 작업도 수행되지 않음)
...
def apply_setting(key, value):
    ...
    # 설정이 고정되어 있으면 업데이트하지 않음
    if shared.cmd_opts.freeze_settings:
        return gr.update()

    # 모델 해시가 프롬프트에 있는 경우 모델을 자동으로 변경하지 않음
    if key == "sd_model_checkpoint" and opts.disable_weights_auto_swap:
        return gr.update()
    
    # 주어진 키에 대한 가장 적합한 체크포인트 찾기
...
def create_output_panel(tabname, outdir):
    # 주어진 탭 이름 및 출력 디렉터리를 사용하여 출력 패널 생성
...
#ordered_ui_categories(): 이 함수는 사용자가 선호하는 순서에 따라 정렬된 사용자 인터페이스(UI) 카테고리를 생성합니다. 
#이 함수는 shared.opts.ui_reorder에서 사용자가 선호하는 순서를 읽고 user_order라는 사전을 생성합니다. 
#그런 다음 shared.ui_reorder_categories를 반복하여 원하는 순서로 카테고리를 산출합니다.

def ordered_ui_categories():
        # 카테고리 이름을 키로, 주문 인덱스를 값으로 사용하여 사전을 만듭니다.
    user_order = {x.strip(): i * 2 + 1 for i, x in enumerate(shared.opts.ui_reorder.split(","))}

    # 사용자의 순서 기본 설정에 따라 정렬된 shared.ui_reorder_categories를 반복합니다.\
    for i, category in sorted(enumerate(shared.ui_reorder_categories), key=lambda x: user_order.get(x[1], x[0] * 2 + 0)):
        # 원하는 순서대로 카테고리를 생성합니다.
        yield category



#get_value_for_setting(키): 이 함수는 키를 입력으로 받아 특정 설정에 대한 업데이트된 값을 반환합니다. 
#이 함수는 getattr(opts, key)를 사용하여 설정의 현재 값을 검색하고 opts.data_labels[key]에서 관련 정보를 가져와서 수행합니다. 
#그런 다음 이 함수는 컴포넌트 인수를 업데이트하여 'precision' 키를 필터링한 다음 gr.update() 메서드를 사용하여 업데이트된 값을 반환합니다.
def get_value_for_setting(key):
    # 주어진 키를 사용하여 설정의 현재 값을 가져옵니다.
...
#create_override_settings_dropdown(탭명, 행): 이 함수는 UI에서 설정을 재정의할 수 있는 드롭다운 메뉴를 생성합니다. 
#탭 이름과 행을 인자로 받아 지정된 속성을 가진 gr.Dropdown 엘리먼트를 생성합니다. 
#또한 드롭다운 메뉴에 대한 변경 이벤트를 설정하여 선택한 항목 수에 따라 드롭다운의 가시성을 업데이트합니다.
#gr.Dropdown 요소는 빈 옵션 목록, "설정 재정의" 레이블, 초기 숨김 상태(표시됨=False), 입력 탭 이름에 기반한 요소 ID, 
#여러 옵션을 선택할 수 있는 기능(다중 선택=True)을 사용하여 만들어집니다.
...
def create_ui():
    ...
    # 텍스트-이미지 모드용 스크립트 초기화하기

    modules.scripts.scripts_current = modules.scripts.scripts_txt2img
    modules.scripts.scripts_txt2img.initialize_scripts(is_img2img=False)

    # gr.Blocks를 사용하여 기본 UI 레이아웃 만들기
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        # 텍스트 프롬프트, 버튼, 토큰 카운터가 포함된 UI의 맨 위 행을 만들고 구성합니다.
        ...
        # 프롬프트 이미지 업로드를 위한 숨겨진 파일 입력 컴포넌트 만들기
        ...
        # 설정 카테고리별로 다양한 UI 컴포넌트와 이벤트 핸들러를 생성하고 구성합니다.
